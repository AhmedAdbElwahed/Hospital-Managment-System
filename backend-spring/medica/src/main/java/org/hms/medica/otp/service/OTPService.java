...
public class OTPService {
  ...
  public OTP crateOTP(User user) {
    OTP otpEntity =
        OTP.builder().otp(generateOTP()).createdAt(LocalDateTime.now()).user(user).build();
  ...
  public boolean verifyOTP(String otp) {
    ...
    LocalDateTime expirationTime =
        otpEntity.getCreatedAt().plusMinutes(10); // Assuming OTP expires after 10
    // minutes
  ...
  public OTP getOTP(String otp) {
    return otpRepository
        .findByOtp(otp)
        .orElseThrow(() -> new RuntimeException(String.format("OTP %s not found!", otp)));
  }
