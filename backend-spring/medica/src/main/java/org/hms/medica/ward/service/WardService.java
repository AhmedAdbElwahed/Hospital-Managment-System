...
public class WardService {
  ...
  public Ward admitToWard(Admission admission, Long wardId) {
    Ward ward = findWardById(wardId);
    if (!isWardFull(ward)) {
      ward.getPatients().add(admission.getPatient());
      return wardRepository.save(ward);
    } else throw new RuntimeException("Can not admit patient to ward");
  }

  public List<WardResponseDto> getAllWards() {
    return wardRepository.findAll().stream().map(wardMapper::mapWardToWardResponseDto).toList();
  }
  ...
  public Ward findWardById(Long wardId) {
    return wardRepository
        .findById(wardId)
        .orElseThrow(() -> new RuntimeException("Can't find ward with id: " + wardId));
  }
