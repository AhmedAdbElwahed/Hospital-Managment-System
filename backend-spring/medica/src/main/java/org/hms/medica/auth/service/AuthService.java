...
public class AuthService {
    ...
    @Transactional
    public void adminRestPassword(AdminResetPassword resetPassword) {
        User user =
                userRepository
                        .findUserByEmail(resetPassword.getEmail())
                        .orElseThrow(() -> new UsernameNotFoundException("User not found!"));
        if (user != null) {
            ...
            if (oldPassword.equals(passwordEncoder.encode(resetPassword.getOldPassword()))) {
                user.setPassword(passwordEncoder.encode(resetPassword.getNewPassword()));
                userRepository.save(user);
            }
        } else
            throw new RuntimeException("Can not change password");
    }
